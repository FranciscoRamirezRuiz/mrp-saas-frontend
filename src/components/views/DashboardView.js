...
const KPICard = ({ title, value, icon: Icon, color, tooltip }) => (
    <div className="bg-white p-4 rounded-xl shadow-md flex items-center justify-between border border-gray-200 h-full" title={tooltip}>
        <div>
...
const SalesChart = ({ data, color }) => (
    <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={data} margin={{ top: 5, right: 20, left: -10, bottom: 20 }}>
            ...
            <YAxis />
            <Tooltip />
            <Area type="monotone" dataKey="sales" name="Unidades Vendidas" stroke={color} fill={color} />
        </AreaChart>
    </ResponsiveContainer>
);

const ABCAnalysisChart = ({ data }) => (
    <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} layout="vertical" margin={{ top: 20, right: 20, left: 20, bottom: 5 }}>
            <XAxis type="number" hide />
            <YAxis type="category" dataKey="name" hide />
            <Tooltip formatter={(value, name) => [value, 'SKUs']} />
            <Bar dataKey="value" name="Nº de SKUs" background={{ fill: '#eee' }}>
...
const DashboardView = () => {
    ...
    // Estados que se guardan en LocalStorage
    ...
    const fetchData = useCallback(async (days) => {
        try {
            ...
            const [kpisRes, salesRes, inventoryRes] = await Promise.all([
                ...
                fetch(`${API_URL}/dashboard/sales-over-time?days=${days}`),
                fetch(`${API_URL}/dashboard/inventory-analysis`),
            ]);
            ...
            setWidgetData({ 
                kpis, 
                sales: sales.map(d => ({ ...d, date: new Date(d.ds).toLocaleDateString('es-ES', { month: 'short', day: 'numeric' }) })),
                inventory
            });
            setError('');
        } catch (err) {
    ...
    useEffect(() => {
        ...
        localStorage.setItem('widgetColors', JSON.stringify(widgetColors));
        localStorage.setItem('widgetNames', JSON.stringify(widgetNames));
    }, [dashboards, widgetColors, widgetNames]);
    ...
    const handleLayoutChange = (layout, newLayouts) => {
        setDashboards(dashboards.map(d => d.id === activeDashboardId ? { ...d, layouts: newLayouts } : d));
    };
    ...
    const handleAddDashboard = () => {
        setPromptModalState({
            ...
            message: 'Introduce un nombre para la nueva pestaña.',
            inputType: 'text',
            initialValue: `Dashboard ${dashboards.length + 1}`,
            onSubmit: (name) => {
                if (name) {
    ...
    const handleRemoveDashboard = (idToRemove) => {
        ...
        setPromptModalState({
            ...
            message: `¿Estás seguro de que quieres eliminar "${dashboardToRemove.name}"?`,
            confirmText: 'Eliminar',
            onSubmit: () => {
                const newDashboards = dashboards.filter(d => d.id !== idToRemove);
                setDashboards(newDashboards);
                if (activeDashboardId === idToRemove) {
    ...
    const handleRenameWidget = (widgetId) => {
        ...
        setPromptModalState({
            isOpen: true,
            title: 'Renombrar Widget',
            message: 'Introduce el nuevo nombre para el widget.',
            inputType: 'text',
            initialValue: currentName,
            onSubmit: (newName) => {
                if (newName) {
                    setWidgetNames({ ...widgetNames, [widgetId]: newName });
                }
            }
        });
    };
    ...
    const handleRemoveWidget = (widgetId) => {
        setDashboards(currentDashboards => currentDashboards.map(dashboard => {
            if (dashboard.id === activeDashboardId) {
                const newLayouts = {};
                Object.keys(dashboard.layouts).forEach(breakpoint => {
                    newLayouts[breakpoint] = dashboard.layouts[breakpoint].filter(item => item.i !== widgetId);
                });
                return { ...dashboard, layouts: newLayouts };
            }
            return dashboard;
        }));
    };
    ...
    const handleAddWidget = (widgetId) => {
        setDashboards(currentDashboards => currentDashboards.map(dashboard => {
            if (dashboard.id === activeDashboardId) {
                ...
                if (lgLayout.some(item => item.i === widgetId)) return dashboard;
                const newWidgetLayout = { i: widgetId, x: (lgLayout.length * 2) % 4, y: Infinity, ...WIDGET_DEFINITIONS[widgetId].defaultLayout };
                const newLayouts = { ...currentLayouts };
                const breakpoints = ['lg', 'md', 'sm', 'xs', 'xxs'];
                breakpoints.forEach(bp => { newLayouts[bp] = [...(currentLayouts[bp] || []), newWidgetLayout]; });
                return { ...dashboard, layouts: newLayouts };
            }
            return dashboard;
        }));
    ...
    const renderWidget = (widgetId) => {
        ...
        return (
            <Card title={title} className="h-full flex flex-col relative group">
                ...
                <div className="drag-handle w-full text-center text-gray-300 cursor-move"><ChevronsRightLeft size={16} className="inline-block"/></div>
    ...
    return (
        <div className="p-8">
            ...
            >
                {(currentDashboard?.layouts?.lg || []).map(widget => (
                    <div key={widget.i}>
